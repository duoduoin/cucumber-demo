package com.gp.cucumber.controller;

import static org.junit.Assert.assertEquals;

import com.gp.cucumber.SpringIntegrationTest;

import io.cucumber.java.en.Then;
import io.cucumber.java.en.When;
import io.cucumber.java8.En;

public class CalculatorControllerStep extends SpringIntegrationTest implements En{

	int response = 0;
    String url = DEFAULT_URL + "calc/";
    
//    public CalculatorControllerStep() {
//    	When("the client calls \\/calc\\/add with values {int} and {int}", (Integer int1, Integer int2) -> {
//    		response = restTemplate.getForObject(url + "add?a=" +
//                    int1 + "&b=" + int2, Integer.class);
//    	});
//
//    	Then("the client receives answer as {int}", (Integer result) -> {
//    		assertEquals(result, response);
//    	});
//
//    	When("the client calls \\/calc\\/sub with values {int} and {int}", (Integer int1, Integer int2) -> {
//    		response = restTemplate.getForObject(url + "sub?a=" +
//                    int1 + "&b=" + int2, Integer.class);
//    	});
//
//    	When("the client calls \\/calc\\/div with values {int} and {int}", (Integer int1, Integer int2) -> {
